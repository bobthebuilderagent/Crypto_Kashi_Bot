...
const DB_PATH = path.join(__dirname, 'crypto_kashi_bot.db');
...
console.log('Tables created:', db.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all().map(t => t.name).join('\n'));

// Verify counts
const tables = ['users', 'settings', 'cex_connections', 'dex_connections', 'prediction_connections', 'crypto_bots', 'trades', 'prediction_markets', 'prediction_positions', 'bot_logs', 'crypto_assets'];
tables.forEach(t => {
